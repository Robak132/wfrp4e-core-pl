...
let god = await ValueDialog.create({text : "Wybierz Bóstwo", title :  "Błogosławieństwo"})

if (god)
{
    let prayers = await warhammer.utility.findAllItems("prayer", "Ładuję błogosławienia...", true, ["system.type.value", "system.god.value"])
    ...
    if (this.item.name.includes("Dowolna"))
        this.item.updateSource({name: this.item.name.replace("Dowolna", god)});
    else
